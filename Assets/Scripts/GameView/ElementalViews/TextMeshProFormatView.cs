...
namespace Game.View
{
    ...
    public class TextMeshProFormatView<T> : MonoBehaviourView<T>
    {
        ...
        {
            await LMotion.Create(baseColor, transparent, transitionTime).WithEase(Ease.InSine).BindToColor(text).ToUniTask();
        }

        protected override async UniTask DoInit(T value)
        {
            text.text = string.Format(format, value);
            var x = LMotion.Create(transparent, baseColor, transitionTime).WithEase(Ease.InSine).BindToColor(text).ToUniTask();
            var y = LMotion.Punch.Create(1.0f, 0.2f, transitionTime / 2.0f).WithFrequency(20)
